...
class CourseManager {
    ...
    static async loadAiRecommendations(track) {
        ...
        try {
            // Always call real recommendations; add track if provided
            const base = API_BASE || 'http://localhost:3000/api';
            const endpoint = `${base}/ai/recommendations${track ? `?track=${encodeURIComponent(track)}` : ''}`;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // AI track tabs events
    const trackTabs = document.querySelectorAll('.ai-track-tab');
    if (trackTabs.length) {
        trackTabs.forEach(tab => {
            tab.addEventListener('click', () => {
                trackTabs.forEach(t => t.classList.remove('active'));
                tab.classList.add('active');
                CourseManager.loadAiRecommendations(tab.dataset.track);
            });
        });
        // Load default active tab
        const active = document.querySelector('.ai-track-tab.active') || trackTabs[0];
        if (active) CourseManager.loadAiRecommendations(active.dataset.track);
    } else {
        // Fallback: load without track if no UI available
